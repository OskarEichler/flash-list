...
export function useRecyclerViewController(recyclerViewManager, ref, scrollViewRef, scrollAnchorRef) {
    ...
    const activeScrollRequest = useRef(null);
    const scrollCommandId = useRef(0);
    const pendingEndCompletions = useRef(new Set());
    useLayoutEffect(() => {
        const endCompletions = pendingEndCompletions.current;
        return () => {
            var _a;
            pendingScrollCallbacks.current = [];
            (_a = activeScrollRequest.current) === null || _a === void 0 ? void 0 : _a.complete();
        ...
        if (recyclerViewManager.getIsFirstLayoutComplete() &&
            ...
            if (firstVisibleItemKey.current) {
                ...
                const currentIndexOfFirstVisibleItem = (_a = recyclerViewManager
                    ...
                    firstVisibleItemKey.current)) !== null && _a !== void 0 ? _a : (hasDataChanged || recyclerViewManager.ignoreScrollEvents
    ...
    const handlerMethods = useMemo(() => {
        return {
                    ...
                    if (scrollViewRef.current &&
                        ...
                        runSafely(() => {
                            ...
                            const performScrollStep = (currentStep) => {
                                ...
                                updateScrollOffsetWithCallback(nextOffset, () => runSafely(() => {
                                    ...
                                    if (index >= recyclerViewManager.getDataLength()) {
                                        ...
                                        release();
                                        return Promise.resolve(handlerMethods.scrollToEnd({ animated })).then(resolve, reject);
                                    }
             ...
             */
            scrollToItem: async ({ item, animated, viewPosition, viewOffset, }) => {
                const { data } = recyclerViewManager.props;
